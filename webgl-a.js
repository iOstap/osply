...
if (window.innerWidth >= 992) {
  ...
  canvas.height = canvas.clientHeight;
    ...
    SIM_RESOLUTION: 256,
    DYE_RESOLUTION: 2000,
    DENSITY_DISSIPATION: .990,
    VELOCITY_DISSIPATION: .90,
    PRESSURE_ITERATIONS: 10,
    SPLAT_RADIUS: 4.5 / window.innerHeight,
    WAVE_INTENSITY: 0.2,
    color: { r: 0.97, g: 0.975, b: 0.94 }
  };
  ...
  window.setTimeout(() => { pointer.firstMove = true; }, 0000);
  ...
  canvas.addEventListener("click", (e) => {
    const offsetTop = getCanvasOffset();
    pointer.dx = 9;
    pointer.dy = 9;
    pointer.x = e.pageX;
    pointer.y = e.pageY - offsetTop;
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    pointer.dy = 5 * (e.pageY - offsetTop - pointer.y);
    pointer.x = e.pageX;
    pointer.y = e.pageY - offsetTop;
    pointer.firstMove = true;
  });
  ...
  canvas.addEventListener("touchmove", (e) => {
    ...
    const offsetTop = getCanvasOffset();
    pointer.moved = true;
    pointer.dx = 7 * (e.targetTouches[0].pageX - pointer.x);
    pointer.dy = 7 * (e.targetTouches[0].pageY - offsetTop - pointer.y);
    pointer.x = e.targetTouches[0].pageX;
    pointer.y = e.targetTouches[0].pageY - offsetTop;
    pointer.firstMove = true;
  });
  ...
  function render() {
    ...
    if (!pointer.firstMove) {
      pointer.moved = true;
      const newX = (0.5 + .2 * Math.cos(.006 * prevTimestamp) * Math.sin(.008 * prevTimestamp)) * window.innerWidth;
      const newY = (0.1 + .12 * Math.sin(.01 * prevTimestamp)) * window.innerHeight;
      pointer.dx = 5 * (newX - pointer.x);
      pointer.dy = 5 * (newY - pointer.y);
      pointer.x = newX;
      pointer.y = newY;
    }
    ...
    if (pointer.moved) {
      ...
      gl.uniform3f(
        ...
        (1. - params.color.r) * params.WAVE_INTENSITY,
        (1. - params.color.g) * params.WAVE_INTENSITY,
        (1. - params.color.b) * params.WAVE_INTENSITY
      );
