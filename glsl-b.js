function injectBShaders() {
  const shaders = [
    ...
    {
      id: "b-fragShaderGradientSubtract",
      type: "x-shader/x-fragment",
      source: `
        precision highp float;
        precision highp sampler2D;
    ...
    },
    {
      ...
      source: `
        ...
        vec2 get_img_uv() {
            float scale = 0.8;
            float canvas_ratio = 5.0 / 6.0;
            float image_ratio = 4.0 / 5.0;

            float scale_x = scale;
            float scale_y = scale * (image_ratio / canvas_ratio);

            vec2 offset = vec2((1.0 - scale_x) * 0.5, (1.0 - scale_y) * 0.5);
            return (vUv - offset) / vec2(scale_x, scale_y);
        }
  ...
  shaders.forEach(({ id, type, source }) => {
